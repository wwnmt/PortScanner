...
# service_list powered by https://github.com/OpenFireTechnologies/PortScanner.git
# add some port
def service(port):
...
def scan_alive():
    ...
    while 1:
        ...
        try:       
            ...
            screenLock.acquire()
            print '[+] Success: host', t_host + ' is up'
            alive_list.append(t_host)
        except:
            screenLock.acquire()
        finally:
            screenLock.release()
...
if __name__ =='__main__':
    ...
    parser.add_option("-t", "--threads", dest="num",help="Max threads, default 50")
    parser.add_option("-H", "--host", dest="host",help="ip,10.10.10.130 or 10.10.10.0/24")
    parser.add_option("-p", "--portlist", dest="ports",help="port list, example: 21,22,23,25 ...")
    
# Get parameter
#t: 10.10.10.0/24 or 10.10.10.130
# thread: int
# ports_list: [start, end] or [port1, port2 ...]
# ip_list: [10.10.10.0 ... 10.10.10.255]
    (options, args) = parser.parse_args()
    if options.num != None and options.num:
        num = int(options.num)
    else :
        num = 100
    if len(args) < 1:
        parser.print_help()
        exit()
    if len(args) < 3 and options.ports != None:
        print '[!] Error: You must specify start port and end port'
        exit()        
    if len(args) == 3 and options.ports == None:
        start_port = int(args[1])
        end_port = int(args[2])
        if start_port > end_port:
            print '[!] Error: end_port bigger than start_port'
            exit()
        ports_list = [i for i in xrange(start_port, end_port+1)]
    if options.ports != None:
        ports_list = [int(port) for port in options.ports.split(',')]     
    ip_list = []
    host = args[0]
    try:
        inet_aton(host)
        ...
        if not validateCIDRBlock(host):
            print '[!] Error: IP address not valid!'
            sys.exit()
        else:
            ip_list = listCIDR(host)
...
# start scanning
    ...
    print '\n[*] Testing: Start Scanning With',num,'Threads' \
        ' at', time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(start_time)) +'\n'
    
    #test host alive or not
    ip_queue = Queue.Queue()
    for ip in ip_list:
        ip_queue.put(ip)
    alive_list = []
    print '[*] Testing: Start test host:',args[0]
    for i in range(num):
        t = threading.Thread(target=scan_alive)
        t.setDaemon(1)    
        t.start()
    ip_queue.join()
